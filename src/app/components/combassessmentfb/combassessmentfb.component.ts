...
@Component({
  selector: 'app-combassessmentfb',
  templateUrl: './combassessmentfb.component.html',
  styleUrls: ['./combassessmentfb.component.scss'],
})
export class CombassessmentfbComponent implements OnInit {
  ...
  submit_answer(data: any) {
    if (data.answer == '') {
      ...
      for (let k = 0; k < this.arrAssessmentTypeFB.length; k++) {
        if (this.arrAssessmentTypeFB[k].question_id == data.question_id) {
          this.arrAssessmentTypeFB[k].is_correct = 1;
        }
      }
      console.log(this.arrObjAssessment);
      if (this.arrAssessmentTypeFB.length == this.arrObjAssessment.length) {
  ...
  on_typing_answer(question_id: number) {
    for (let k = 0; k < this.arrAssessmentTypeFB.length; k++) {
      if (this.arrAssessmentTypeFB[k].question_id == question_id) {
        this.arrAssessmentTypeFB[k].is_correct = 0;
      }
    }
  }
  ...
  ngOnInit(): void {
    ...
    if (sess_info != null) {
      ...
      if (index == -1) {
        console.log('Ques List  :', this.arrAssessmentTypeFB);
        console.log('Student ID : ', this.arrKeyObj.student_id);
        this.isSubmitted = false;
        for (let k = 0; k < this.arrAssessmentTypeFB.length; k++) {
          this.arrAssessmentTypeFB[k].answer = '';
        }
      } else {
        this.isSubmitted = true;
